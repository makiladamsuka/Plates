...
export const Dock: React.FC<DockProps> = ({ activeTab, onTabChange, isDark = true }) => {
  ...
  return (
    <div className="fixed bottom-3 left-0 right-0 max-w-[412px] mx-auto px-4 z-40">
      <nav className={`h-[58px] rounded-full shadow-2xl flex items-center justify-around px-3 backdrop-blur-xl border transition-colors duration-200 ${
        isDark
          ? 'bg-[#121318]/95 border-white/[0.08]'
          : 'bg-[#1a1a1a]/95 border-black/10'
      }`}>
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className={`text-[10px] mt-0.5 tracking-tight font-medium ${isActive ? 'text-white font-semibold' : 'text-neutral-400'}`}>
                {item.label}
              </span>
              {isActive && (
